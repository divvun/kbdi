...
enum Opt {
    ...
    RegistryRegen,
    #[structopt(name = "language_enable", about = "Enable a language with provided tag")]
    LanguageEnable {
        /// Language tag in BCP 47 format (eg: sma-Latn-NO)
        tag: String,
    },
    ...
    #[structopt(name = "language_list", about = "Lists all languages enabled for the current user")]
    LanguageList,
    #[structopt(name = "keyboard_list", about = "Lists all enabled keyboards for the user")]
    KeyboardList,
    #[structopt(name = "keyboard_enabled", about = "Lists all enabled keyboards for the user")]
    KeyboardEnabled,
    ...
    match opt {
        Opt::KeyboardInstall{tag, layout, guid, dll, lang, enable} => {            
            ...
            if enable {
                ...
                keyboard::enable(&tag, &guid, lang.as_deref(), false).unwrap();
            }
        },
        Opt::KeyboardUninstall{guid} => {
            keyboard::uninstall(&guid).unwrap();
        },
        Opt::KeyboardEnable{tag, guid, lang, default_user} => {
            keyboard::enable(&tag, &guid, lang.as_deref(), default_user).unwrap();
        },
        Opt::RegistryRegen => {
